...
按[初始化模块](../skills/drama-studio/references/initialization.md)先探测实际权限，再选择脚本、已有连接或文件入口。宿主名称不证明可执行后台任务；无持久运行环境就不能把对话内代码执行当成日更服务。选择时核对时区、休眠行为、重入保护和任务回执，未支持项标待配置。

初始化记录与项目分开保存在用户目录，技能升级不重建已有数据或定时任务。当前仓库提供引导协议和记录模板，没有跨平台定时注册器；不得凭本文猜测宿主命令或宣称计划已生效。

## 可选 Yoki 红果榜单

上游[红果组件](https://github.com/yokichen9149-byte/yoki-data-analysis/tree/main/mcp/hongguo-rank)提供本地 SQLite、采集／查询 CLI 和可选 stdio MCP。采集可直接调用脚本，不需要安装或启动 MCP。支持本地命令型 MCP 的宿主可复用既有部署；先确认实际路径、依赖与连接，不因安装本技能启动第二套服务或日更任务。查询不会自动采集，新用户没有历史数据时不能获得过去多日趋势。
