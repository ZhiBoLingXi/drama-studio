...
该连接不是安装前置条件。用户已有时按[接入说明](skills/drama-studio/references/hongguo-rank.md)发现工具并只读查状态；没有连接则记“可选数据源未配置”，不判主技能安装失败。用户明确要求新增本地服务时再按[宿主指南](docs/hosts.md)核对现有部署，避免重复数据库或定时器。网页可读取用户提供的导出文件，不把本地stdio配置填成远程MCP地址。
